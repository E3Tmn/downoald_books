...
def download_picture(count, book_cover_url, book_url):
    ...
    image_path = os.path.join(image_folder, f'{count}{ext}')
...
def download_book(count, book_title, book_folder, id):
    ...
    book_path = os.path.join(book_folder, f'{count}.{book_name}')
...
def main():
    ...
    parser.add_argument('--end_page', type=int, default=10, help='Номер второй страницы')
        ...
        try:
            ...
            for tag in href_tags:
                try:
                    ...
                    if not skip_txt:
                        book_path = download_book(count, book['book_title'], dest_folder, re.findall(r'\d+', book_link)[0])
                        book['book_path'] = book_path.replace("\\", "/")
                    if not skip_imgs:
                        image_path = download_picture(count, book['book_cover_url'], link)
                        book['image_path'] = image_path.replace("\\", "/")
                    books.append(book)
